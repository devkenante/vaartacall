...
object KenanteMessageHandler : KenanteCallEventListener {
    ...
    override fun onSessionCreated(sessionId: BigInteger) {
        ...
        var position=BigInteger.valueOf(0L)
        KenanteWsSendMessages.attachJanusPlugin(sessionId, true ,"0")
    }

    override fun onPluginAttached(handleId: BigInteger, isPublisher: Boolean, userId: String) {
    ...
    override fun onUnpublished(userId: String) {
    ...
    override fun onLeaving(userId: String) {
        for ((key, value) in KenanteSession.pluginHandles) {
            if (value.id .equals( userId)) {
                KenanteSendCallbacks.getInstance()
    ...
    private fun createPluginHandler(handleId: BigInteger, isPublisher: Boolean, userId: String) {
        if (isPublisher) {
