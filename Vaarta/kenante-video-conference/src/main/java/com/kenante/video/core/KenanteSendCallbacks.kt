...
class KenanteSendCallbacks {
    ...
    fun sendUserCallEventCB(event: UserCallEvent, userId: String) {
        handler.post {
            when (event) {
    ...
    fun sendMediaStreamMediaFlowCallEventCB(event: MediaEvent, userId: BigInteger, mediaType: MediaType) {
        handler.post {
            when (event) {
