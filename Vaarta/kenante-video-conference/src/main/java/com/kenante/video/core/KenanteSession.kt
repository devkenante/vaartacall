...
class KenanteSession : KenanteWsConnEventListener {
    ...
    fun createSession(sessionEventListener: SessionEventListener) {
        //Release old session
        if (sessionOngoing)
            releaseSession()
    ...
    fun startCall(roomId: Long, audio: Boolean, video: Boolean, bitrate: KenanteBitrate) {
        this.roomId = roomId
        this.audio = audio
        this.video = video
        this.bitrate = bitrate
        for ((_, value) in pluginHandles) {
                val role = UserRoles.publisher
                ...
                KenanteWsSendMessages.joinRoom(
                        ...
                        this.audio,
                        this.video,
                        KenanteMessageParser.currentuserid?.toBigInteger()
                )
    ...
    fun subscribeToPublisher(userId: String) {
