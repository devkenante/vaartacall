...
class KenantePluginHandler : CameraVideoCapturer.CameraEventsHandler {
    ...
    var handleId =BigInteger.valueOf(0L)
    ...
    var trickle = true
    var publisher_id=BigInteger.valueOf(0L)
    ...
    internal fun startSendingMediaStream() {
        if (!isPublisher!!)
            return
        if (video!!) {
            Log.e(TAG,"Start Sending Video Stream")
           // KenanteMediaStreamManager.GetManager().setVideoCapturer(KenanteCamera.GetCamera())
        }
        if (!audio!!) {
            //KenanteMediaStreamManager.GetManager().setAudioTrack
            //addLocalStream()
            Log.e(TAG,"Start Sending Audio Stream")
        }
    }
        ...
        pc = mediaStreamManager.peerConnectionFactory?.createPeerConnection(
                ...
                object : PeerConnection.Observer {
                    ...
                    override fun onRemoveStream(p0: MediaStream?) {
                        ...
                        mediaStreamManager.removeAudioTrack(KenanteMessageParser.currentuserid.toString())
                        mediaStreamManager.removeVideoTrack(KenanteMessageParser.currentuserid.toString())
                    }
    ...
    fun startThisUser() {
        KenanteWsSendMessages.startSubscriber(
                ...
                handleId,
                localJsep,
                KenanteSession.getInstance().roomId.toInt()
        )
    }
...
private fun KenanteWsSendMessages.configureUser(sessionId: BigInteger, handleId: BigInteger, jsep: JSONObject, audio: Boolean?, video: Boolean) {
